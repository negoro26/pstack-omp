...
Transcripts live at `~/.omp/agent/sessions/<encoded-cwd>/<ISO-timestamp>_<uuid>.jsonl` (subagent artifacts sit beside it as `<session>/<AgentName>.jsonl`), where `<slug>` is the workspace path with $HOME stripped, a leading dash, and each "/" turned into "-" (so `$HOME` becomes `-` and `~/.omp/pstack` maps to `sessions/-.omp-pstack`).
