...
5. Verify on the matching surface. Run the typecheck and lint lane with `lsp` action `diagnostics` (`file` set to `"*"` for a whole-workspace check, a concrete file or glob for a scoped one) alongside the test lane, and require the new code to be clean before the surface check; when the `lsp` tool is off (`lsp.enabled`), fall back to the project's own typecheck and lint commands. "Inconclusive" or wrong-surface is not a pass; flag it.
