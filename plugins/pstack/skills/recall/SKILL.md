...
Transcripts live at `~/.omp/agent/sessions/<encoded-cwd>/<ISO-timestamp>_<uuid>.jsonl`, where `<encoded-cwd>` is the cwd with $HOME stripped and each "/" turned into "-" (so `~/proj` becomes `-proj`). Every line is one JSON object. The first is a `type:title` record and the rest are messages.
