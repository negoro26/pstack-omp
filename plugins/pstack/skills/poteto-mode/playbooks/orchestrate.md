...
- **Worker / verifier.** Always `isolated: true` unless the task needs this machine: `browser` or `computer` for UIs, or `hub` process ops plus bash for CLIs and TUIs. Reading local transcripts under `~/.omp/agent/sessions/<encoded-cwd>/`. Simulators and local IDE state. Auth that exists only here. An isolated worker runs on this machine in its own worktree with no conversation history, so its brief inlines what it needs or points at absolute paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the isolated default and the shared-checkout exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (hold it under a `hub` watcher or a systemd timer, with a long fallback heartbeat), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder in the standing orders. Restacks run in an isolated subagent with its own worktree, never in the parent checkout.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, agent state from `hub` `op: "list"` and `hub` `op: "jobs"`. Transcript mtime is not liveness.
...
- An omp restart stops every agent. Resuming the session rebuilds its subagents as parked rows that `hub` `op: "send"` revives, except isolated ones, which leave only a `history://<id>` transcript. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach pushed work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write. `orch` replaces a lock whose holder pid is gone.
