...
**Defaults for every `Task` call.** one `task` call with all items in `tasks[]` (batched in parallel), full tools per spawn, file pointers not inlined context, a capability named per role and never a vendor slug (configurable via `/setup-pstack`; defaults your fast code model for code, your prose model for prose and your strongest judgment model for judgment). Resolve the capability at run time against what `omp models` lists. Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model when the task needs judgment or the intent is vague, and to your strongest instruction-following model when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to your fast code model. The chat model is the operator's choice, made with `/model`, and no skill overrides it. A per-agent model is `task.agentModelOverrides` in `~/.omp/agent/config.yml`, keyed by agent name, and its value may be a capability alias the operator binds in `modelRoles`; `/setup-pstack` owns that configuration. The `task` tool takes no `model` argument, so read its schema before assuming a per-call field and treat the keyed override as the only per-spawn lever. The `task` tool takes no `readonly` argument either, and the per-item `tools` field only exposes eval-defined kernel tools to a spawn, so a read-only worker is posture carried in the brief, never a wire field. The one per-spawn restriction omp enforces is `tools` in an agent's frontmatter, which binds to an agent definition, not a task call. A role with no override entry runs on the parent chat model, which is the correct default; a role line of `inherit-parent` or `auto` states that explicitly (leave that role out of `task.agentModelOverrides`). A role that needs its own model needs its own thin agent file plus an override entry, so a model race needs one agent file per arm. Per-role lines in the `/setup-pstack` rule override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line keeps its default.
=======
