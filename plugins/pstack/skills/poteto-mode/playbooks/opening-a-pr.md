...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as `browser`, `computer`, `hub` plus bash, or the targeted tests. State the outcome of each check, not only the command name.
=======
...
**Babysit.** Opening a PR does not start a babysit. Post the URL and keep building. Finish the phase or stack first. Run a separate babysit pass through the **Babysit** playbook (`playbooks/babysit.md`) only when the user asks for one after the whole stack exists. A babysit for each new PR stalls the build and spends checks on commits that later waves restart. Push back when feedback drifts from intent.

A subagent that opens a PR runs `interrogate`, the `unslop` skill (`skill://unslop`) plus `omp cleanse`, and `/no-comments`. It returns the URL and does not babysit. Return to the parent.
