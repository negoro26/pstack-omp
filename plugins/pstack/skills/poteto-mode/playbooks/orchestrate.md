...
- **Worker / verifier.** Always `isolated: true` unless the task needs this checkout or local state: `browser`/`computer` or `hub`+bash runtime verification. Reading local transcripts under the session transcript tree `~/.omp/agent/sessions/<encoded-cwd>/*.jsonl` (subagents: `<session>/<AgentName>.jsonl`). Simulators and local IDE state. Auth that exists only here. An isolated worker runs on this machine but in its own worktree and starts with no conversation history, so it sees neither your uncommitted files nor its siblings. Its brief inlines what it needs or points at absolute store paths. Prefer fewer, broader workers. One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and agent state from `hub op:"list"` and `hub op:"jobs"`. Transcript mtime is not liveness.
