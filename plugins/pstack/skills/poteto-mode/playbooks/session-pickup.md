...
<<<<<<< HEAD
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a `history://<id>` or `agent://<id>` handoff, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. A local transcript under the session transcript tree `~/.omp/agent/sessions/<encoded-cwd>/*.jsonl` (subagents: `<session>/<AgentName>.jsonl`) (the system prompt names the path; stay inside your own `<encoded-cwd>` directory, since globbing sibling `<encoded-cwd>` directories crosses workspace boundaries and reads private sessions from unrelated projects), a prior agent's `history://<id>` or `agent://<id>`, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
=======
