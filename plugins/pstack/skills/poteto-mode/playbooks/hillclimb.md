...
5. Loop, one hypothesis per iteration:
   ...
   Each iteration ends in a check before the next begins (the **sequence-verifiable-units** principle skill). If the run is unattended, borrow only the wake mechanism from the Autonomous run playbook (`skill://poteto-mode/playbooks/autonomous-run.md`), not its stop rule.
